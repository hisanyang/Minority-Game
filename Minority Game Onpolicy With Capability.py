# -*- coding: utf-8 -*-
"""
Created on Sun Nov 23 11:58:03 2025
...
INITIAL_CAPITAL = 1000.0  # Large to avoid early exits
STAY_HOME_REWARD = 0.0
STAY_HOME_CHOICE_NAME = "Stay Home"

# --- Capability Dynamics ---
INITIAL_CAPABILITY = 10.0
CAP_GROWTH_ON_WIN_MULT = 0.10   # multiplicative growth: cap *= (1 + 0.10) on win
CAP_DECAY_ON_LOSS_MULT = 0.05   # multiplicative decay: cap *= (1 - x) on loss (set to 0 to disable)
MIN_CAPABILITY = 1e-6
MAX_CAPABILITY = 1e6            # prevent runaway
# Optional small drift to prevent freezing (set to 0 to disable)
CAP_DECAY_PER_ROUND = 0.00      # e.g., 0.005 means cap *= (1 - 0.005) each round regardless
...
class Agent:
    ...
    def update_strategy_scores(self, attendance_history, bars_config, choice_names, experienced_reward, used_strategy_name, policy_type):
        """
        Updates strategy scores based on outcome.
        ON_POLICY: only the used strategy is credited with the experienced reward.
        OFF_POLICY: simple heuristic — still credit used strategy; optionally, you can
                    add heuristics for others, but here we keep it simple/neutral.
        """
        if not self.is_active:
            return
        # If agent was committed, no strategy was used.
        if not used_strategy_name:
            return

        if policy_type == 'ON_POLICY':
            self.strategy_scores[used_strategy_name] += experienced_reward
        else:
            # Keep it simple: used strategy gets full reward, others get none.
            self.strategy_scores[used_strategy_name] += experienced_reward

    def apply_capability_drift(self):
        if CAP_DECAY_PER_ROUND > 0.0:
            self.capability = max(MIN_CAPABILITY, self.capability * (1.0 - CAP_DECAY_PER_ROUND))
...
class ElFarolMultiBarSimulation:
    def __init__(self, bars_config, allow_stay_home):
        ...
        if self.allow_stay_home:
            self.choice_names = self.bar_names + [STAY_HOME_CHOICE_NAME]
        else:
            self.choice_names = self.bar_names
        ...
        for r in range(NUM_ROUNDS):
            ...
            for agent in active_agents:
                ...
                if agent.commitment_remaining > 0:
                    ...
                    if strategy_obj:
                        choice = strategy_obj.predict(self.attendance_history, self.bars_config, self.choice_names)
                    else:
                        choice = random.choice(self.choice_names)
            ...
            # Attendance counts and attendee lists per bar
            attendance = {name: 0 for name in self.bar_names}
            bar_attendees = {name: [] for name in self.bar_names}
            stayers = 0
            for agent in active_agents:
                choice, _ = agent_choices[agent.id]
                if choice in self.bar_names:
                    attendance[choice] += 1
                    bar_attendees[choice].append(agent.id)
                elif choice == STAY_HOME_CHOICE_NAME:
                    stayers += 1

            # Resolve outcomes per agent using capability-weighted selection
            agent_outcomes = {agent.id: STAY_HOME_REWARD for agent in active_agents}
            winners_per_bar = {name: [] for name in self.bar_names}  # optional tracking

            for bar in self.bars_config:
                name = bar['name']
                capacity = bar['capacity']
                reward_success = bar['reward_success']
                reward_fail = bar['reward_fail']
                attendees = bar_attendees[name]
                n_att = len(attendees)
                ...
                if n_att <= capacity:
                    ...
                    # Weighted sampling without replacement based on capability
                    weights = np.array([max(MIN_CAPABILITY, self.agents[i].capability) for i in attendees], dtype=float)
                    total_w = weights.sum()
                    if total_w <= 0:
                        # Fallback to uniform if somehow zero
                        winners = list(np.random.choice(attendees, size=capacity, replace=False))
                    else:
                        probs = weights / total_w
                        winners = list(np.random.choice(attendees, size=capacity, replace=False, p=probs))
                    winners_set = set(winners)
                    losers = [i for i in attendees if i not in winners_set]

                winners_per_bar[name] = winners

                for i in winners:
                    agent_outcomes[i] = reward_success
                for i in losers:
                    agent_outcomes[i] = reward_fail

            # Apply capital, capability updates, and strategy score updates
            for agent in active_agents:
                choice, chosen_strategy_name = agent_choices[agent.id]
                reward = agent_outcomes[agent.id]
                agent.update_capital(reward)

                # Capability updates: only if the agent attended a bar
                if choice in self.bar_names:
                    if reward > 0:
                        agent.capability *= (1.0 + CAP_GROWTH_ON_WIN_MULT)
                    elif reward < 0 and CAP_DECAY_ON_LOSS_MULT > 0.0:
                        agent.capability *= (1.0 - CAP_DECAY_ON_LOSS_MULT)

                    # Clamp capability
                    agent.capability = float(np.clip(agent.capability, MIN_CAPABILITY, MAX_CAPABILITY))

                # Per-round drift/decay (optional)
                agent.apply_capability_drift()

                # Strategy score updates
                agent.update_strategy_scores(
                    self.attendance_history,
                    self.bars_config,
                    self.choice_names,
                    experienced_reward=reward,
                    used_strategy_name=chosen_strategy_name,
                    policy_type=POLICY_TYPE
                )

            # Log histories
            for name in self.bar_names:
                self.attendance_history[name].append(attendance[name])
    ...
    def _fill_remaining_history(self, current_round):
        ...
        for _ in range(num_rounds_to_pad):
            ...
            for name in self.bar_names:
                self.attendance_history[name].append(0)
...
class EnsembleRunner:
    ...
    def run_all_simulations(self):
        ...
        for i in range(self.num_runs):
            ...
            for name in self.bar_names:
                self.all_attendance_histories[name].append(attend_hist[name])
            self.all_stayer_histories.append(stayer_hist)
            self.all_total_attendance_histories.append(total_attend_hist)
            if (i + 1) % 10 == 0 or self.num_runs == 1:
                print(f"  ... Run {i+1}/{self.num_runs} complete.")
    ...
    def perform_statistical_tests(self):
        ...
        for bar in self.bars_config:
            ...
            self.statistical_results[bar_name] = {
                "mean_attendance": np.mean(all_attendance_data),
                "capacity": bar_capacity,
        ...
        if len(self.bar_names) == 2:
            ...
            if bar1_data and bar2_data and len(bar1_data) == len(bar2_data):
                ...
                self.paired_test_results = {
                    "bar1_name": bar1_name,
                    "bar2_name": bar2_name,
                    "bar1_mean": np.mean(bar1_data),
                    "bar2_mean": np.mean(bar2_data),
                    "t_statistic": t_statistic,
                    "p_value": p_value,
                }
    ...
    def display_statistical_results(self):
        if self.statistical_results:
            ...
            for bar_name, results in self.statistical_results.items():
                ...
                conclusion = "FAIL to reject H0" if results['p_value'] >= 0.05 else "REJECT H0 (Significant)"
        ...
        if self.paired_test_results:
            ...
            conclusion = "FAIL to reject H0" if res['p_value'] >= 0.05 else "REJECT H0 (Significant)"
    ...
    def analyze_and_display_survivors(self):
        if not self.final_survivor_counts:
            print("No survivor data to analyze.")
            return
        mean_survivors = np.mean(self.final_survivor_counts)
        median_survivors = np.median(self.final_survivor_counts)
        std_survivors = np.std(self.final_survivor_counts)
        min_survivors = np.min(self.final_survivor_counts)
        max_survivors = np.max(self.final_survivor_counts)
        stay_home_status = "ENABLED" if self.allow_stay_home else "DISABLED"
        ...
        if self.num_runs == 1:
            ...
            plt.xlabel('Number of Survivors at End of Simulation', fontsize=12)
            plt.ylabel('Frequency (Number of Runs)', fontsize=12)
            plt.xlim(-0.5, NUM_AGENTS + 0.5)
            plt.legend()
            plt.grid(True)
            plt.show()
...
if __name__ == '__main__':
    ...
    print(f"Capability: INIT={INITIAL_CAPABILITY}, GROW_ON_WIN={CAP_GROWTH_ON_WIN_MULT}, DECAY_ON_LOSS={CAP_DECAY_ON_LOSS_MULT}, DRIFT={CAP_DECAY_PER_ROUND}")
    print("="*60)
